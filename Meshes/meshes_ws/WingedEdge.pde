class WingedEdge {
  ...
  ArrayList<ArrayList<Integer>> edgeFace;
  ...
  void build() {
    ...
    edgeFace = new ArrayList<ArrayList<Integer>>();
    ...
    for(int i=0; i<shape.getChildCount(); i++){
      ...
      for(int j=0; j<face.size(); j++){
        ...
        if(!edge.contains(edge)){
          edges.add(edge);
          ArrayList<Integer> listEdgeFaces = new ArrayList<Integer>();
          listEdgeFaces.add(faces.size() - 1);
          edgeFace.add(listEdgeFaces);
        }else{
          int index = edges.indexOf(edge);
          edgeFace.get(index).add(faces.size() - 1);
        }
    ...
    for(ArrayList<Integer> facesList : edgeFace){
      for(Integer face : facesList){
